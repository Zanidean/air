...
get_enrolment <-
  function(measures,
    ...
    `Calculated Full-Time for Year` <-
      "[Measures].[Calculated Full Time for Year]"
    `Calculated Part-Time for Year`  <-
      "[Measures].[Calculated Part Time for Year]"
    ...
    `Legal Status` <-
      "[Legal Status].[By Legal Status Group].[Legal Status]"
    `Age Group` <- "[Age Group].[By Age Group].[Age Group]"
    Age <- "[Age Group].[By Age Group].[Age]"

    `Source Country` <-
      "[Source Country].[Source Country].[Source Country]"
    `Aboriginal Indicator` <-
      "[Aboriginal Indicator].[By Aboriginal Indicator].[Aboriginal Indicator]"
    `Aboriginal Grouping` <-
      "[Aboriginal Indicator].[By Aboriginal Grouping].[Aboriginal Grouping]"
    `Country Of Citizenship` <-
      "[Country Of Citizenship].[Country Of Citizenship].[Country Of Citizenship]"
    Language <- "[Language].[Language].[Language]"
    `Grade Completed Year` <-
      "[Grade Completed Year].[By Grade Completed Year].[Grade Completed Year]"
    `Current Status` <-
      "[Current Status].[Current Status].[Current Status]"
    `Completion Grouping` <-
      "[Completion Status].[By Completion Grouping].[Completion Grouping]"
    ...
    `Level Of Study` <-
      "[Level Of Study].[Level Of Study].[Level Of Study]"
    `Current Status` <-
      "[Current Status].[Current Status].[Current Status]"
    `Registration Status` <-
      "[Registration Status].[By Registration Status].[Registration Status]"
    `Registration Type` <-
      "[Registration Type].[Registration Type].[Registration Type]"
    `Level of Study` <-
      "[Level Of Study].[Level Of Study].[Level Of Study]"
    `Calculated Learned Registration Status` <-
      "[Calculated Learner Registration Status].[Calculated Learner Registration Status].[Calculated Learner Registration Status]"
    ...
    `Provider Location` <-
      "[Provider Location].[By Provider and Location].[Provider Location]"
    `Provider Sector` <-
      "[Provider].[By Current Sector].[Current Sector]"
    `Provider Service Area` <-
      "[Provider].[By Service Area].[Service Area]"


    ###Service Area
    `Country` <- "[Service Area].[By Country and Province].[Country]"
    `Province` <-
      "[Service Area].[By Country and Province].[Province]"
    `Service Area` <-
      "[Service Area].[By Country and Province].[Service Area]"
    `Country and Province` <-
      "[Service Area].[By Country and Province]"
    `Postal Code` <-
    ...
    `Program Name` <-
      "[Program Specialization Name].[By Program Name].[Program Name]"
    `Program Name Code` <-
      "[Program and Specialization].[Program Code].[Program Code]"
    `Program Specialization` <-
      "[Program and Specialization].[Specialization Name].[Specialization Name]"
    `Program Specialization Code` <-
      "[Program and Specialization].[Specialization Code].[Specialization Code]"
    `Program Length` <-
    ...
    axis(qry, 2) <-
      c("NONEMPTY([Academic Year].[Academic Year].[Academic Year].MEMBERS)")
    ...
    pcs <-
      fltr(postalcodes,
           "[Service Area].[By Country and Province].[Postal Code].&[CAME")
    #Giving a filter option for postalcodes in service area
    cds <-
      fltr(
        censusdivisions,
        "[Census Division].[By Country and Province].[Census Division].&[0"
      )
    ...
    cips2 <-
      filtercips(cips_2, "[CIP Level].[By Two Digits].[Two Digit Level].&[")
    cips4 <-
      filtercips(cips_4, "[CIP Level].[By Two Digits].[Four Digit Level].&[")
    cips6 <-
      filtercips(cips_6, "[CIP Level].[By Two Digits].[Six Digit Level].&[")
    ...
    slices <-
      c(
        "([Registration Status].[By Registration Group].[Registration Group].&[1]",
        providers,
        pcs,
        cds,
        cips,
